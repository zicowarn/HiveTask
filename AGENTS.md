...
**规则：单文件/小改动必须用文本工具（Edit/Write）直改；只有跨多文件批量修改、
或需要执行/计算才能完成的变更，才允许用脚本（python/node heredoc 等）。**

- 用脚本做单点编辑会让 diff 审查变难、出错难定位，且绕过工具的防护（文件已读校验等）；
- 批量脚本改完必须用 `git diff` 逐处核对，不得只看脚本输出就当改对了。
...
## 交互反馈节奏（用户定案，2026-09-19）

- **提交必须由用户要求**：助手完成修复并通过验证后**停在未提交状态**，等用户实测验收、
  明确说"提交"才执行 git commit。不得以"顺手提交"为由主动 commit。
- **单点小修要快**：一个交互/显示级别的小 bug，从定位到修完控制在几分钟内——
  只跑受影响的单测文件（不跑全量 gate），探针/实机验证仅在必要时做；
  修完即报告，由用户实测；问题批量反馈、批量修，避免用户为一个小问题等待十几分钟。
- 全量 `pnpm gate` 保留给：用户明确要求的"收尾"、里程碑提交前、或改动横跨多模块时。

## 质量门禁（分层：即时 → 阶段 → 提交）

六步检查（i18n 校验 / ESLint / Vitest / vue-tsc+build / Clippy / cargo test）
按**改动面 + 时机**分档执行；**提交前必须是全量六步**，中间档按需。

| 档 | 命令 | 何时跑 | 内容 |
|---|---|---|---|
| 即时 | `pnpm gate:fast` | 每改完一小步（即上节「单点小修要快」的机制化） | 按 git 改动面自动选最小集：i18n 校验＋改动文件 ESLint＋关联用例 Vitest＋vue-tsc；动了 `src-tauri/` 则追加 cargo check |
| 阶段 | `pnpm gate:web`<br>`pnpm gate:rust` | 一轮收口、交用户实测前 | 六步按侧拆开：前端 1–4 步 / Rust 5–6 步 |
| 提交 | `pnpm gate` | 提交前、里程碑、改动横跨多模块 | 全量六步，任何一步失败即止 |

**红线（任何档位都不省）**：**门禁不绿不提交**；i18n 键对齐与占位符校验（1 秒，
两语言对齐的唯一守卫）；提交前的全量六步；ESLint 对裸字号的告警视同错误处理
（改动面含前端时即时档也会跑到）。

**节奏（2026-09-22 用户定案，同文写入个人全局 AGENTS.md）**：**全量门禁与全量单测
只在提交前跑一次**——中间每一步不要反复跑全量，用即时档（`pnpm gate:fast`，按 git 改动面
自动选最小集）或只跑与改动相关的检查。阶段档（`gate:web` / `gate:rust`）用于「一轮收口、
交用户实测前」，也不是每步都跑。只有两种情况例外：用户明确要求现在跑；改动横跨多模块、
需要立刻确认没有连带破坏。**同一轮里反复全量既慢又掩盖真正的增量信号。**

**可暂缓**：用户要自己实测期间（配合上节「问题批量反馈、批量修」），阶段档可推迟到
收口时补跑。**但暂缓期间不得声称"门禁绿"**——没跑就如实说"未跑"，收口必须补。

- **与 R5 的衔接**：一轮收口的「门禁绿」= 改动面所属**半套**（`gate:web` / `gate:rust`）
  全过且红线未破；提交前与里程碑交付仍以**全量六步**为准。**半套绿不得表述为
  「门禁全绿」**（同 R3 口径：绿 ≠ 形态对）。

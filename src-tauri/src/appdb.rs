...
const CURRENT_APP_SCHEMA_VERSION: i64 = 18;
...
const APP_MIGRATION_005: &str = include_str!("migrations/app_005_project_connection.sql");
...
const APP_MIGRATION_010: &str = include_str!("migrations/app_010_calendar_events.sql");
...
const APP_MIGRATION_018: &str = include_str!("migrations/app_018_project_item_archived.sql");
...
pub(crate) fn app_migrate(conn: &Connection) -> anyhow::Result<()> {
    ...
    if version < 5 {
        conn.execute_batch(APP_MIGRATION_005).context("app 迁移 005 失败")?;
    }
    if version < 6 {
        conn.execute_batch(APP_MIGRATION_006).context("app 迁移 006 失败")?;
    }
    if version < 7 {
        conn.execute_batch(APP_MIGRATION_007).context("app 迁移 007 失败")?;
    }
    if version < 8 {
    ...
    if version < 10 {
        conn.execute_batch(APP_MIGRATION_010).context("app 迁移 010 失败")?;
    }
    if version < 11 {
        conn.execute_batch(APP_MIGRATION_011).context("app 迁移 011 失败")?;
    }
    if version < 12 {
        conn.execute_batch(APP_MIGRATION_012).context("app 迁移 012 失败")?;
    }
    if version < 13 {
    ...
    if version < 18 {
        conn.execute_batch(APP_MIGRATION_018).context("app 迁移 018 失败")?;
    }

 ...
 *
 * 两条约定：
 * - **「适应窗口」= 整幅可见 + 四周留白 5%**（2026-09-19 用户口径，由 10% 收紧到 5%），
 *   取宽高两个方向的较小缩放比；命名沿用 Preview.app 的「适应窗口」。
 *   ⚠️ 留白要真正成立需要两半配合：**缩放比管大小**（这里），**居中管位置**（各渲染器
 *   自己的 flex 容器，见图片分支的 .image-wrap）——只有缩放没有居中时，留白会全部
 *   堆在底部、顶部贴边；
